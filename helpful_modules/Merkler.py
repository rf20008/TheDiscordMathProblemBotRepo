...
class FixedLengthMerkler:
    ...
    def __init__(self, size: int, contents: list[str | bytes] | None):
        ...
        if contents is not None:
            ...
            self._build_from_hashes(sindex=1, sleft=0, sright=self.size-1, data_hashes=contents_hashes)
            #print(self)
    ...
    def _get_idx_logn(self, *, index: int, left: int, right: int, pos: int) -> int:
        ...
        return self._get_idx_logn(index=index*2+1, left=mid+1, right=right, pos=pos)
    ...
    def __getitem__(self, index: int):
        ...
        if index<0 or index>=self.size:
            raise IndexError("FixedLengthMerkler index out of range")
        return self.tree[self._get_idx_logn(index=1, left=0, right=self.size-1, pos = index)]
    ...
    def _verify_whole_tree(self, *, index: int, left: int, right: int, data_blocks: list[str | bytes]):
        """
        ...
        return True
    def _verify_index(self, *, sindex: int, left: int, right: int, index_to_verify: int, should_be_hash: bytes):
        """
    ...
    def verify_index(self, index: int, should_be: bytes | str):
        ...
        return self._verify_index(sindex=1, left=0, right=self.size-1, index_to_verify=index, should_be_hash=should_be)
...
def DynamicLengthMerkler(FixedLengthMerkler):
    ...
    def resize(self, new_size: int):
        ...
        def build_tree(index: int, left: int, right: int):
            ...
            new_tree[index] = self.sha3_hash(new_tree[index * 2] + b'\x00' + new_tree[index * 2 + 1])
