...
class CommandStats:
    ...
    def to_dict(self):
        return {
            "usages": [
                usage.to_dict for usage self.usages
            ],
...
class StreamWrapperStorer:
    ...
    def __init__(self, stream):
        self.stream = stream
    async def writeStats(self, stats: CommandStats):
        await self.stream.write(str(stats))
    async def return_stats(self):
        #assume the stats object is in ONE LINE (not multiple)
        return json.parse(await self.reading.readline())
