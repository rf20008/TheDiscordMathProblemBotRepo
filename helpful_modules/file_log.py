...
class AuditLog:
    ...
    def add_log_entry(
        ...
        log_entry: str,
        priority: int = 3,
        secrecy: SecrecyLevel = SecrecyLevel.MODS_AND_DEVS_ONLY,
    ...
    async def add_log_entry_with_clear(
        self,
        log_entry: str,
        priority: int = 3,
        secrecy: SecrecyLevel = SecrecyLevel.MODS_AND_DEVS_ONLY,
...
class FileLog(AuditLog):
    ...
    def __init__(
        self, filename: str, max_buffer_size: int = 200, overwrite: bool = False
    ):
        if overwrite:
...
class FileDictLog(AuditLog):
    ...
    def __init__(
        self, filename: str, overwrite: bool = False, max_buffer_size: int = 200
    ):
    ...
    def size(self):
        if not self.size_initialized_yet:
            raise RuntimeError(
                "This tree's index has not been initialized yet! Run `await <reference to tree>.init_index()` to initialize it"
            )
    ...
    def set_size(self, new_size: int):
        raise NotImplementedError(
            "Python prohibits me from having async setters to properties!"
        )
    ...
    def add_log_entry(
        ...
        cur_time = time.asctime(time.localtime())
        cur_time_hashed = hex(
            farmhash.FarmHash128(str(cur_time) + str(frac(time.time())))
        )

        self.buffer.append(
            (
                "entry" + str(cur_time) + str(cur_time_hashed),
                {
                    "index": self.size,
                    "cur_time": cur_time,
                    "log_msg": log_entry,
    ...
    async def clear_buffer(self):
        ...
        old_dict.update(
            dict(self.buffer)
        )  # Update the old dictionary with the new logs
...
class AppendingFileLog(AuditLog):
    ...
    def parse_entry(
        self, entry: str
    ) -> tuple[time.struct_time, int, SecrecyLevel, str, dict]:
        ...
        if first_pipe == -1:
            raise ValueError(
                "Malformed log entry -- log entries must have at least 3 pipe characters"
            )
        # step 2: extract priority
        second_pipe = entry.find("|", first_pipe + 1)
        if second_pipe == -1:
            raise ValueError(
                "Malformed log entry. Log entries must have at least 3 pipes"
            )
    ...
    def add_log_entry(
        ...
        self.buffer.append(
            self.format_entry(
                log_entry=log_entry, priority=priority, extra_info=extra_info
            )
        )
