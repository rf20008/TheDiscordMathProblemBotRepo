...
TOLERANCE = 1e-3
FORMAT_HELP = """To answer this question, give your answer in the form var1 var2 var3 ... varN. 
You must put the values of each variable space-separated, without "var1=". Also, each value cannot have any spaces.
For example, if the solution is (var1, var2) = (3,4), the **only** correct answer is "3 4". 
Some teachers may ask you to answer in the form "(var name for all var names) = (ordered tuple)". For example, they may ask you to say "(var1, var2) = (3,4)". 
However, this WILL NOT be accepted!
Also, you cannot enter your answer in any other form. You cannot say "x=3, y=4" or "var1=3, var2=4", etc. They will not be parsed correctly.
Remember that you cannot have **ANY** spaces in any of the tokens!!!!
The reason for this extreme formatting format is that I don't want to have to parse many different formats.
"""
...
class LinearAlgebraProblem(base_problem.BaseProblem):
    ...
    @classmethod
    def from_coefficients(cls, coeffs, equal_to, author_id=845751152901750824, guild_id=None, *args, **kwargs):
        """
        ...
        if num_eqs != len(coeffs):
            raise ValueError("The number of equations does not match the number of coefficient sets")

        num_vars = len(coeffs[0])
        for equation_id in range(len(coeffs)):
            equation = coeffs[equation_id]
            if len(equation) != num_vars:
                raise ValueError("coeffs is not a rectangular matrix. You must pass in a rectangular matrix")
            eq = ""
            for i in range(len(equation)):
                coeff = equation[i]
                if not isinstance(coeff, float) and not isinstance(coeff, numbers.Number) and not isinstance(coeff,
                                                                                                                 mpmath.mpc) and not isinstance(
                        coeff, complex):
                    raise ValueError("A non-numeric value was found. All coefficients must be numbers")
                if coeff == 0:
                    continue
        ...
        return LinearAlgebraProblem.__init__(question, coeffs=coeffs, equal_to=equal_to, id=author_id, guild_id=guild_id,
                                              *args, **kwargs)
    ...
    def check_answer(self, answer):
        ...
        """
        tokens = answer.split(" ")
        tokens = list(map(lambda token: token.replace("i", "j"), tokens))  # remember: we use j for complex numbers in Python
        try:
        ...
        for equation_idx in range(len(self.coeffs)):
            ...
            if len(equation) != len(self.coeffs[0]):
                raise MathProblemsModuleException("The matrix of coefficients is not rectangular")
            if len(values) != len(equation):
                raise MathProblemsModuleException("You don't have enough variable values to satisfy all the equations")
            lhs = 0.0
            for i in range(len(equation)):
