...
DEFAULT_GLOBAL_LIMIT = RateLimit(unit="minute", limit=GLOBAL_LIMIT_PER_MIN, per=ONE_MINUTE)
DEFAULT_RATE_LIMITS = [
...
class RateLimiter: # todo: more rate limits
    ...
    def __init__(
        ...
        if not isinstance(user_limits, list) and not all(isinstance(limit, RateLimit) for limit in user_limits):
            raise TypeError("User limits must be of type list or tuple. Also, all its elements must be of type RateLimit.")
    ...
    async def __call__(self, inter) -> str:
        ...
        while len(self.global_deque)>0 and self.global_deque[0] < cur_time - self.global_limit.per:
