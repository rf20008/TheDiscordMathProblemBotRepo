    ...
    def initialize_bitboards(self):
        ...
        for row in range(self.size**2):
            for col in range(self.size**2):
                if self.row_bit_board[row] & (1 << self.board[row][col]) and self.board[row][col] != 0:
                    raise ValueError(
        ...
        for col in range(self.size**2):
            for row in range(self.size**2):
                if self.col_bit_board[col] & (1<<self.board[row][col]) and self.board[row][col] != 0:
                    raise ValueError(f"sudoku rule violation detected: 2 instances of {self.board[row][col]} in column {col+1}")
        ...
        for boxnum in range(self.size**2):
            ...
            for row in range(self.size):
                for col in range(self.size):
                    ...
                    if self.box_bit_board[boxnum] & (1<<self.board[x][y]) and self.board[x][y] != 0:
                        raise ValueError(f"Sudoku rule violation detected: 2 instances of {self.board[x][y]} in box {boxnum}")
