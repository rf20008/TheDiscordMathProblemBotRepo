...
class TestTokenType(unittest.TestCase):
    def test_as_int_1(self):
        for op, exp_val in [(1, 1), (1.0, 1), (2, 2), (complex(2), 2)]:
        ...
        for operand1, operand2 in (
            ...
            with self.subTest(operand1=operand1, operand2=operand2):
                with self.assertRaises(
                    ArithmeticOverflowError, msg=f"{operand1}**{operand2}"
                ):
    ...
    def test_and_and_or(self):
        for operator_name, operand1, operand2, expected_result in [
            ...
            with unittest.mock.patch(
                "helpful_modules.arithmetic_evaluator.as_int", side_effect=int
            ) as mock_int:
                operator = BinaryOperator(operator_name)
                computed_val = operator(operand1, operand2)
...
class TestToken(unittest.TestCase):
        ...
        for token, should_be_operand in [
            ...
            with self.subTest(token=token, should_be_operand=should_be_operand):
                self.assertEqual(
                    token.is_number(),
                    should_be_operand,
                    msg=f"{token.is_operand()} is not {should_be_operand} as token is {token}",
                )
    ...
    def test_is_parenthesis(self):
        for token, should_be_operand in [
            ...
            with self.subTest(token=token, should_be_operand=should_be_operand):
                self.assertEqual(
                    token.is_parenthesis(),
                    should_be_operand,
                    msg=f"{token.is_parenthesis()} is not {should_be_operand} as token is {token}",
                )
...
class TestTokenizer(unittest.TestCase):
    ...
    def test_invalid_float(self):
        self.assertEqual(
            tokenizeregex("1.2.3"),
            [Token(TokenType.NUMBER, 1.2), Token(TokenType.NUMBER, 0.3)],
        )
    ...
    def test_tokenize_small_expression(self):
        ...
        self.assertEqual(
            TOKENS[1], Token(TokenType.BINARY_OPERATOR, BinaryOperator.PLUS)
        )
...
class TestShuntingYard(unittest.TestCase):
    ...
    def assertRPN(self, tokens, expected_values):
        ...
        result = [
            t.token_value if t.token_type != TokenType.NUMBER else t.token_value
            for t in output
        ]
